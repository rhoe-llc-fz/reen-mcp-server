...
server.tool("get_plan", "Get a specific plan by ID with all tasks and subtasks", {
    ...
    const narData = await client.get(`/api/gant/plans/${plan_id}/narrative`);
...
// Tool: reorder_task
// =============================================
server.tool("reorder_task", "Move a task to a new position within its sibling group", {
...
// Tool: list_artifacts
// =============================================
server.tool("list_artifacts", "List all artifacts for a plan", {
    plan_id: z.string().describe("Plan ID"),
}, async ({ plan_id }) => {
...
// =============================================
server.tool("create_artifact", "Create a new artifact (note/file) in a plan", {
    plan_id: z.string().describe("Plan ID"),
...
// =============================================
server.tool("update_artifact", "Update an artifact's title or content", {
...
// =============================================
server.tool("delete_artifact", "Delete an artifact (soft delete)", {
...
// --- Research tools ---
server.tool("research_upload_book", "Upload a book/document for analysis via JSON. Pipeline extracts text, segments into chapters, then stops at 'segments_ready' for your local AI to analyze.", {
...
server.tool("research_list_books", "List all books in the user's research library with their processing status.", {}, async () => {
    const data = await client.get("/api/research/books");
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("research_get_book", "Get a book's knowledge graph (cards + edges). Only meaningful when status = 'completed'.", {
    book_id: z.string().describe("Book ID"),
}, async ({ book_id }) => {
    const data = await client.get(`/api/research/books/${book_id}`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("research_get_book_status", "Check a book's processing status and progress.", {
    book_id: z.string().describe("Book ID"),
}, async ({ book_id }) => {
...
server.tool("research_submit_analysis", "Submit analysis results (cards + edges) produced by your local AI model. This completes the book processing and builds the knowledge graph. Can be called on 'segments_ready' or 'completed' books (re-submission replaces old data).", {
    book_id: z.string().describe("Book ID"),
    ...
    const body = { cards, edges };
    if (book_summary)
        body.book_summary = book_summary;
    const data = await client.post(`/api/research/books/${book_id}/analyze`, body);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("research_delete_book", "Delete a book from the research library.", {
...
// --- Strategic Plans ---
server.tool("list_strategic_plans", "List all strategic plans for the current user", {}, async () => {
    const data = await client.get("/api/gant/strategic-plans");
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("create_strategic_plan", "Create a new strategic plan (free-form canvas where cards = plans)", {
...
server.tool("get_strategic_plan", "Get a strategic plan with all cards and edges (full canvas)", {
...
server.tool("update_strategic_plan", "Update a strategic plan's title or description", {
    strategic_plan_id: z.string().describe("Strategic plan ID"),
    title: z.string().optional().describe("New title"),
    description: z.string().optional().describe("New description"),
...
server.tool("delete_strategic_plan", "Delete a strategic plan (soft delete)", {
...
server.tool("add_strategic_plan_card", "Add a plan card to the strategic plan canvas", {
    strategic_plan_id: z.string().describe("Strategic plan ID"),
...
server.tool("remove_strategic_plan_card", "Remove a plan card from the strategic plan canvas", {
    strategic_plan_id: z.string().describe("Strategic plan ID"),
...
server.tool("add_strategic_plan_edge", "Add an edge (arrow) between two cards on the strategic plan canvas", {
    strategic_plan_id: z.string().describe("Strategic plan ID"),
    ...
    const body = { source_card_id, target_card_id, edge_type };
    if (label)
        body.label = label;
...
// =============================================
server.tool("search_claims", "Semantic search across 24K+ scientific claims in ARGUS knowledge base. " +
...
server.tool("search_papers", "Semantic search across 3.9K+ scientific papers in ARGUS knowledge base. " +
...
server.tool("get_paper_details", "Get full details for a specific paper by arXiv ID: summaries (4 languages), " +
