...
server.tool(
  "upload_exhelp_file",
  "Upload a file to an Ex-Help request. The file content is passed as base64-encoded string. Supported: md, txt, json, yaml, csv, py, js, ts, patch, diff, sh, sql, toml.",
  {
  ...
  async ({ plan_id, exhelp_id, filename, content_base64, mime_type }) => {
    ...
    const data = await client.upload(
      `/api/gant/plans/${plan_id}/files`,
      { name: filename, content, mimeType: mime_type },
      { context: "exhelp", exhelp_id },
    );
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  },
);
